...
plus an 8-page ACM-\texttt{acmart} sigconf paper.
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.1em]
  ...
  \item \textbf{Research paper}~--- an 8-page ACM \texttt{acmart} sigconf
        write-up in \texttt{task2-final-project/report-latex/report.pdf}.
  \item \textbf{Plugin code}~---
        \texttt{code/src/cost\_aware\_eviction/}, 1{,}074 lines. Heart
        is \texttt{eviction\_manager.py}~--- an indexed min-heap with
        $O(\log n)$ priority updates, thread-safe via a single
        \texttt{Lock}.
  \item \textbf{Test suite}~--- \textbf{304 tests}, all
...
\texttt{zilliztech/GPTCache}; the write-up is an 8-page ACM sigconf
paper.
\end{quote}
